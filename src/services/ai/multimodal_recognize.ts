...
const fileRecognizeResultSchema = z.object({
    ...
    texts: z.array(z.object({
      left: z.number({message: 'left coordinate of the text in PX, in the whole image'}),
      top: z.number({message: 'top coordinate of the text in PX, in the whole image'}),
      ...
      comment: z.string({message: 'additional comment of the text, or the translation'}),
    }))
})

export async function recognizeFile(apiKey: string, conf: MultimodalModelConf, msg: UserMessage, abortSignal?: AbortSignal): Promise<z.infer<typeof fileRecognizeResultSchema>> {
  const generateConf: GenerateObjectOptions<typeof fileRecognizeResultSchema> = {
    messages: [
      { content: 'You are a helpful assistant. Please do as user instructs.', role: 'system' },
      msg,
    ], 
